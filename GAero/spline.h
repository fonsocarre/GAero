...
std::vector<double> spline::operator()(const std::vector<double>& xVec) const
{
    const int N = static_cast<int>(xVec.size());
    std::vector<double> yVec(N);
    for (int i=0; i<N; ++i)
    {
        yVec[i] = this->operator()(xVec[i]);
    }
    return yVec;
}
...
{
    tk::spline s;
    s.set_points(xPointsIn, yPointsIn);
    
    yPointsOut = s (xPointsOut);
}
